...
def buscar_pets():
    ...
    todas_os_pets = database.search(
        Query().fragment(query)
    )
    return jsonify(
        Pets(
            pets=todas_os_pets,
            count=len(todas_os_pets)
        ).dict()
    )
...
## -------->>   POST   <<--------   
@server.post('/pets')
@spec.validate(body=Request(Pet), resp=Response(HTTP_200=Pet) ) # 'HTTP_200=Pet' Lê-se que o OK dessa resposta 200 é a classe Pet
def inserir_pet():
